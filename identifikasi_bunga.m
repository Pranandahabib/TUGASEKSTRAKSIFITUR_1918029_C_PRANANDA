...
% handles    structure with handles and user data (see GUIDATA)
try
    %berfungsi untuk memanggil gambar dengan format JPG
    [nama_file,nama_path] = uigetfile({'*.*'});
     %apabila File yang dipilih benar maka akan langsung menjalankan printah if di bawah ini 
    if ~isequal(nama_file,0)
        % membaca cintr rgb
        Img = imread(fullfile(nama_path,nama_file));
        %menampilkan gambar pada axes
        axes(handles.axes1)
        imshow(Img)
        handles.Img = Img;
        guidata(hObject,handles)
...
% handles    structure with handles and user data (see GUIDATA)
try
    %gambar yg di panggil pada bttn 1 akan d proses pda bttn 2
...
% handles    structure with handles and user data (see GUIDATA)
try
    set(handles.edit1,'String',['Rose'])
    ciri_bunga = handles.ciri_bunga;
    load mdl
    jadi = round(sim(net,ciri_bunga));
    %jika pada tampilan runing nanti yang terdeteksi  kode di bawah makan
    %akan mucul bunga 
    if jadi == [1;0;0;0;0]
        kelas = 'Daisy';
    ...
    elseif jadi == [0;0;0;0;1]
        ...
        %dan apabila citra yang dimasukkan salah maka akan tidak dikenali
...
%jika ingin menggati gambar maka kita akan menjalankan buttn 4 ntuk mereset
%citra
axes(handles.axes1)
cla reset
set(gca,'XTick',[])
